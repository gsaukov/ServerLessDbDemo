...
vpc_id=$(aws ec2 describe-vpcs --filters "Name=isDefault, Values=true" | jq '.Vpcs[0].VpcId')
...
aws cloudformation wait stack-create-complete --stack-name cf-network --template-url https://s3.amazonaws.com/my-new-lambda-bucket-6328764287365/cf-network.yaml --region=us-east-1 --parameters ParameterKey=VPCID,ParameterValue=${vpc_id}
aws cloudformation wait stack-create-complete --stack-name cf-aurora-serverless --template-url https://s3.amazonaws.com/my-new-lambda-bucket-6328764287365/cf-aurora-serverless.yaml --region=us-east-1
aws cloudformation wait stack-create-complete --stack-name cf-sldb-initializer --template-url https://s3.amazonaws.com/my-new-lambda-bucket-6328764287365/cf-sldb-initializer.yaml --region=us-east-1
